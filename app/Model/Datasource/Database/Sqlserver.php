...
class Sqlserver extends DboSource {
	...
	const SP_ADD_EXTENDED_PROPERTY = "sys.sp_addextendedproperty";
	const SP_UPDATE_EXTENDED_PROPERTY = "sys.sp_updateextendedproperty";
 ...
 */
	public $tableParameters = array(
		'comment' => array('value' => 'comment', 'quote' => false, 'join' => '=', 'column' => 'comment')
	);
 ...
 */
	public $columns = array(
		'primary_key' => array('name' => 'IDENTITY (1, 1) NOT NULL PRIMARY KEY'),
 ...
 */
	public function describe($model) {
		...
		while ($column = $cols->fetch(PDO::FETCH_OBJ)) {
			...
			while ($extProperty = $extProperties->fetch(PDO::FETCH_OBJ)) {
				$fields[$field]['comment'] = $extProperty->value;
				break;
			}
	...
	public function renderStatement($type, $data) {
		switch (strtolower($type)) {
			...
			case "schema":
				...
				foreach (array('columns') as $var) {
					if (is_array(${$var})) {
						${$var} = "\t" . implode(",\n\t", array_filter(${$var}));
					}
				}
				foreach (array('indexes') as $var) {
					if (is_array(${$var})) {
						${$var} = "\t" . implode("\n\t", array_filter(${$var}));
					}
				}
 ...
 */
	public function index($model) {
		...
		while ($index = $indexes->fetch(PDO::FETCH_OBJ)) {
			$indexName = $index->name;
			$indexIsPrimary = $index->is_primary_key;
			$indexIsUnique = $index->is_unique_constraint;
			$indexObjectId = $index->object_id;
			...
			$indexColumns = $this->_execute(
				"SELECT
					COL.name,
					INDCOL.is_descending_key,
					INDCOL.index_column_id
				FROM sys.index_columns AS INDCOL
				INNER JOIN sys.columns AS COL
					ON INDCOL.object_id = COL.object_id and INDCOL.column_id = COL.column_id
				WHERE INDCOL.object_id = $indexObjectId AND INDCOL.index_id = $indexId
				ORDER BY
					INDCOL.index_column_id"
			);

			while ($indexColumn = $indexColumns->fetch(PDO::FETCH_OBJ)) {
				$columnName = $indexColumn->name;
				$columnAscDesc = (!$indexColumn->is_descending_key) ? 'ASC' : 'DESC';

				$tableIndexes[$indexName]['column'][] = $columnName;
			}
			if (!empty($tableIndexes)) {
				if (count($tableIndexes[$indexName]['column']) === 1) {
					$tableIndexes[$indexName]['column'] = $tableIndexes[$indexName]['column'][0];
				}
				$tableIndexes[$indexName]['unique'] = $indexIsUnique;
				if ($indexIsPrimary) {
					$tableIndexes[$indexName]['unique'] = 1;
				}
			}
		...
		while ($foreignKey = $foreignKeys->fetch(PDO::FETCH_OBJ)) {
			$foreignKeyTable = $foreignKey->table_name;
			$foreignKeyName = $foreignKey->foreign_key;

			$foreignKeyColumns = $this->_execute(
				"SELECT
    				tab1.name AS table_name,
					obj.name AS foreign_key,
				...
				WHERE tab1.name = '$foreignKeyTable' and obj.name = '$foreignKeyName'"
			);

			while ($foreignKeyColumn = $foreignKeyColumns->fetch(PDO::FETCH_OBJ)) {
				$columnName = $foreignKeyColumn->column_name;

				$tableIndexes[$foreignKeyName]['column'][] = $columnName;
			}
			if (count($tableIndexes[$foreignKeyName]['column']) === 1) {
				$tableIndexes[$foreignKeyName]['column'] = $tableIndexes[$foreignKeyName]['column'][0];
			}
			$tableIndexes[$foreignKeyName]['unique'] = 0;

			$foreignKeyColumns->closeCursor();
		}
		$foreignKeys->closeCursor();
		...
		foreach ($indexes as $name => $value) {
			if ($name === 'PRIMARY') {
				...
				if (isset($value['unique']) && $value['unique']) {
					...
					$out = "CREATE INDEX {$name} ON {$table} ";
				}
...
/**
 * Generate a Sqlserver Alter Table syntax for the given Schema comparison
 *
 ...
 */
	public function alterSchema($compare, $table = null) {
		...
		foreach ($compare as $curTable => $types) {
			...
			if (!$table || $table === $curTable) {
					...
					switch ($type) {
						case 'add':
							foreach ($column as $field => $col) {
								...
								$extProperties[] = $this->_buildCommentExtProperty($curTable, $col, 'add');
							}
							break;
						case 'drop':
							foreach ($column as $field => $col) {
						...
						case 'change':
							foreach ($column as $field => $col) {
								...
								$extProperties[] = $this->_buildCommentExtProperty($curTable, $col, 'change');
							}
							break;
					}
				}
				...
				$indexList = $this->_alterIndexes($curTable, $indexes);
				if (!empty($indexList)) {
					$out .= implode(",\n\t", $indexList) . ";\n\n";
				}
				...
				$out .= $this->_tableComment($curTable, $tableParameters);
			}
		}
		return $out;
	}
	...
	/**
	 * Generates the table comment query
	 *
	 * @param string $tableName The table name.
	 * @param null|array $tableParameters The table parameter array.
	 * @return string The generated query.
	 */
	protected function _tableComment($tableName, $tableParameters = null) {
		return ' /* TODO Table comment  */';/*
		...
		foreach ($tableParameters as $type => $tableParam) {
			if (key($tableParam) !== 'comment') {
				continue;
			}

			$comment = current($tableParam);

			if ($type === 'add') {
				$method = self::SP_ADD_EXTENDED_PROPERTY;
			} elseif ($type === 'change') {
				$method = self::SP_UPDATE_EXTENDED_PROPERTY;
			}

			$statement .= "EXEC " . $method .
	...
	/**
	 * Generate SQL Server index alteration statements for a table.
	 *
	...
	protected function _alterIndexes($table, $indexes) {
		...
		if (isset($indexes['drop'])) {
			foreach ($indexes['drop'] as $name => $value) {
				if ($name === 'PRIMARY') {
					$out = "ALTER TABLE $table DROP
						(SELECT name
							FROM   sys.key_constraints
							WHERE  [type] = 'PK'
       						AND [parent_object_id] = Object_id('dbo.$table');";
				} else {
					$out = "ALTER TABLE $table DROP $name";
				}
				$alter[] = $out;
			}
		}
...
/**
 * Builds the statement for manipulating the comment extended property.
 *
 * Currently it only adds a comment ("MS_Description").
 * It should also covered the situation when there is already a comment.
 *
 * @param string $curTable The table
 * @param array $field The column data.
 * @param string $type The type of modification, either 'add' or 'change'.
 * @return string The extended property build statement.
 */
	protected function _buildCommentExtProperty($curTable, $field, $type) {
		if (in_array($field['name'], array('indexes', 'tableParameters'))) {
			return '';
		}
		...
		if (empty($field['comment'])) {
			...
			$comment = preg_replace('/(\')/m', '\'\'', $field['comment']);
		}
		...
		EXEC sys.sp_addextendedproperty
			...
			@level0name=N'dbo',
			@level1type=N'TABLE',
			@level1name=N'$curTable',
			@level2type=N'COLUMN',
			@level2name=N'" . $field['name'] . "';";
		}
